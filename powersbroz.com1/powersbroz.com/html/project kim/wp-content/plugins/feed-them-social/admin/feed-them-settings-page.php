...
class FTS_settings_page {
  ...
  <div class="fts-content">
   ...
   <h2 style="border-top:0px; margin-bottom:4px !important;"><?php _e('Custom Date and Time', 'feed-them-social'); ?></h2> <div><?php if($ftsCustomDate !== '' || $ftsCustomTime !== '') { echo date(get_option( 'fts-custom-date' ) .' '.get_option( 'fts-custom-time' )); } ?></div>
    <p style="margin:12px 0 !important;"><input name="fts-custom-date" style="max-width:105px;" class="fts-color-settings-admin-input" id="fts-custom-date" placeholder="<?php _e('Date', 'feed-them-social'); ?>" value="<?php echo get_option('fts-custom-date'); ?>" />
    <input name="fts-custom-time" style="max-width:75px;" class="fts-color-settings-admin-input" id="fts-custom-time" placeholder="<?php _e('Time', 'feed-them-social'); ?>" value="<?php echo get_option('fts-custom-time'); ?>" /></p>
   <div><?php _e('This will override the date and time format above.', 'feed-them-social'); ?><br/><a href="https://codex.wordpress.org/Formatting_Date_and_Time" target="_blank"><?php _e('Options for custom date and time formatting.', 'feed-them-social'); ?></a></div>
		</div>
		...
		function updateTextArea_fb_page() {
			...
			if(is_plugin_active('feed-them-social-facebook-reviews/feed-them-social-facebook-reviews.php')) {
			 ...
			 if(is_plugin_active('feed-them-social-facebook-reviews/feed-them-social-facebook-reviews.php')) { 
				...
				<?php } ?>
						if (albums_photos_option == "album_photos") {
			  				var final_fb_page_shortcode = '[fts_facebook' + fb_page_id + fb_album_id + fb_feed_type + image_width + image_height + space_between_photos + hide_date_likes_comments + center_container + image_stack_animation + position_lr + position_top +']';
						}
						else if (albums_photos_option == "albums") {
			  				var final_fb_page_shortcode = '[fts_facebook' + fb_page_id + fb_feed_type + image_width + image_height + space_between_photos + hide_date_likes_comments + center_container + image_stack_animation + position_lr + position_top +']';
						}
						else if (albums_photos_option == "page") {
							var final_fb_page_shortcode = '[fts_facebook' + fb_page_id + fb_page_posts_displayed + facebook_height_final + fb_feed_type + ']';
						}
						<?php if(is_plugin_active('feed-them-social-facebook-reviews/feed-them-social-facebook-reviews.php')) { 
						include(WP_CONTENT_DIR.'/plugins/feed-them-social-facebook-reviews/admin/js/facebook-page-reviews-js.js');  ?>
				<?php } ?>
						else {
							var final_fb_page_shortcode = '[fts_facebook' + fb_page_id + facebook_height_final + fb_feed_type + ']';
						} 
		<?php } ?>
		...
		function updateTextArea_twitter() {
			...
			if(is_plugin_active('feed-them-premium/feed-them-premium.php')) {
			   include(WP_CONTENT_DIR.'/plugins/feed-them-premium/admin/js/twitter-settings-js.js');
					...
					var final_twitter_shorcode = '[fts_twitter' + twitter_name  + twitter_search_final + twitter_height_final + ']';
		<?php } ?>
			...
			if(is_plugin_active('feed-them-premium/feed-them-premium.php') && WP_CONTENT_DIR.'/plugins/feed-them-premium/admin/js/vine-settings-js.js') {
			   include(WP_CONTENT_DIR.'/plugins/feed-them-premium/admin/js/instagram-settings-js.js');
					...
					if (jQuery("select#instagram-custom-gallery").val() == "no") {
			  			var final_instagram_shorcode = '[fts_instagram' + instagram_id + instagram_feed_type + instagram_page_height_final +']';
					}
					else {
						var final_instagram_shorcode = '[fts_instagram' + instagram_id + super_gallery + image_size + icon_size + space_between_photos + hide_date_likes_comments + center_container + image_stack_animation + instagram_feed_type + instagram_page_height_final +']';
					}
		<?php } ?>
