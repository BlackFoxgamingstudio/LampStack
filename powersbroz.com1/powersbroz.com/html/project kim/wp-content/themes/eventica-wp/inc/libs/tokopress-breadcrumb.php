<?php
/**
 * Functions to handle breadcrumb.
 *
 * WARNING: This file is part of the core TokoPress framework.
...
class Breadcrumb_Trail {
	...
	public function trail() {
		...
		if ( !empty( $this->items ) && is_array( $this->items ) ) {
			...
			$breadcrumb = "\n\t\t" . '<' . tag_escape( $this->args['container'] ) . ' class="breadcrumb-trail breadcrumb breadcrumbs">';
		...
		if ( true === $this->args['echo'] )
			...
			return wp_kses_post( $breadcrumb );
	}
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => __( 'Browse:',                 'tokopress' ),
			'home'                => __( 'Home',                    'tokopress' ),
			'search'              => __( 'Search results for "%s"', 'tokopress' ),
			'error_404'           => __( '404 Not Found',           'tokopress' ),
			'paged'               => __( 'Page %d',                 'tokopress' ),
			'archives'            => __( 'Archives',                'tokopress' ),
			'archive_minute_hour' => __( 'g:i a',                   'tokopress' ),
			'archive_minute'      => __( 'Minute %d',               'tokopress' ),
			'archive_hour'        => __( 'g a',                     'tokopress' ),
			'archive_day'         => __( 'd',                       'tokopress' ),
			'archive_week'        => __( 'Week %d',                 'tokopress' ),
			'archive_month'       => __( 'F',                       'tokopress' ),
			'archive_year'        => __( 'Y',                       'tokopress' ),
		//	'edit'                => __( 'Edit',                    'tokopress' ), // @todo Implement edit link
		);
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( __( 'Y', 'tokopress' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_year'], $post_id ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y', 'tokopress' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_month'], $post_id ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y', 'tokopress' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_day'], $post_id ) . '</a>';
