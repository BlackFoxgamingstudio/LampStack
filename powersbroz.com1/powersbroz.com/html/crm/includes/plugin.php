...
class module_base{
	...
	function get_menu($holding_module=false,$holding_page=false,$type=false){
		...
		foreach($links as $link_id => $link){
			...
			$links[$link_id]['order'] = !empty($links[$link_id]['order']) ? $links[$link_id]['order'] : module_config::c('_menu_order_'.$this->module_name,isset($link['order']) ? $link['order'] : $this->module_position);
            if(!$links[$link_id]['order']){
                $links[$link_id]['order'] = isset($link['order']) ? $link['order'] : $this->module_position;
            }
			// icon name overwriting from config option.
			if(!empty($links[$link_id]['icon_name'])){
				$new_icon = module_config::c('_menu_icon_'.$this->module_name,$links[$link_id]['icon_name']);
				if($new_icon){
					$links[$link_id]['icon_name'] = $new_icon;
				}
			}
