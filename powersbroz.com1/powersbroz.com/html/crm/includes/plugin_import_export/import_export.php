            ...
            if(is_array($rows) && count($rows)){
                ...
                if(class_exists('module_extra',false) && isset(self::$pagination_options['extra']) && count(self::$pagination_options['extra'])){
	                ...
	                foreach(self::$pagination_options['extra'] as $extra_field_id => $extra_field_settings) {
		                ...
		                $sql = "SELECT `extra_default_id`,`extra_key`, `order`, `display_type`, `owner_table`, `searchable`, `field_type`, `options` FROM `"._DB_PREFIX."extra_default` e  WHERE e.owner_table = '" . mysql_real_escape_string( $extra_field_settings['owner_table'] ) . "' ORDER BY e.`order` ASC";
                    ...
                    if(class_exists('module_extra',false) && isset(self::$pagination_options['extra']) && count(self::$pagination_options['extra'])){
	                    ...
	                    foreach(self::$pagination_options['extra'] as $extra_field_id => $extra_field_settings) {
	                        ...
	                        if(isset($row[$extra_field_settings['owner_id']]) && $row[$extra_field_settings['owner_id']] > 0){
	                            $sql = "SELECT `extra_key` AS `id`, `extra` FROM `"._DB_PREFIX."extra` WHERE owner_table = '".mysql_real_escape_string($extra_field_settings['owner_table'])."' AND `owner_id` = '".(int)$row[$extra_field_settings['owner_id']]."'";
	                            $extra_vals = qa($sql);
	                        }
