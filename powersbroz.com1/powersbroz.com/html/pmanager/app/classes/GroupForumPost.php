<?php
/**
 * @package Entity Group Forum Post Class and Methods
 * @version 1.0
 * @date 23 August 2015
...
class GroupForumPost implements ForumPostInterface {
    ...
    private static $table = "group_forum_posts";

    // Constructor functions
    public function __construct($record) {
        $this->id 	        = $record['id'];
        $this->post_object  = Group::find('id', $record['groupid']);
    ...
    public function build_replies() {
        $replies = gen_query("SELECT * FROM group_forum_posts WHERE isreply = 1 AND replyto = ".$this->id);
        if ($replies['count'] > 0) {
