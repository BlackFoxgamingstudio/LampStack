...
class Installer
{
  ...
  public $load; 
	...
	function check_config()
	{
		...
		{
			$_check = $this->_test_dir(ROOTPATH . $folder, true);
	...
	function _save_database_settings()
	{
		...
		// Try create and save config/congig.file
		$config = array(
			'encryption_key' => $this->generateEncryptKey(),
			'base_url' => BASEURL
		);
		...
		{
			$this->_send_error('database', __('Connection to the database fails with the provided settings.'), $_POST);
		}
	...
	function _test_dir($dir, $recursive = false)
	{
		...
		if ( ! is_really_writable($dir) OR !$dh = opendir($dir)) @chmod($dir, 0777);
		if ( ! is_really_writable($dir) OR !$dh = opendir($dir)) return false;
		if ($recursive)
		{
