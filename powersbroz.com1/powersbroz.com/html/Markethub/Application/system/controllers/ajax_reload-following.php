<?php
	...
	if ($errored == 1) {
		...
		$r = $this->db2->query('SELECT iduser, code, firstname, lastname, username, avatar, num_items, num_followers, num_following, validated FROM users, relations WHERE iduser=leader AND subscriber='.$iduser.' ORDER BY rltdate DESC LIMIT '.$numitems.','.$C->NUM_FOLLOWING_PAGE);
