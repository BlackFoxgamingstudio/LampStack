<?php
	...
	if ($errored == 1) {
		...
		$totalitems = $this->db2->fetch_field('SELECT count(idlike) FROM likes WHERE iduser='.$iduser);
		
		$r = $this->db2->query('SELECT idlike, items.*, users.username FROM likes, items, users WHERE likes.typeitem=2 AND likes.iduser='.$iduser.' AND likes.iditem=items.iditem AND items.iduser=users.iduser ORDER BY datewhen DESC LIMIT '.$numitems.','.$C->NUM_FAVORITES_PAGE);
