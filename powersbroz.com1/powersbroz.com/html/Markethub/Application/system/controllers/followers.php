<?php
	...
	if ($D->show_profile==1) {
		...
		$r = $this->db2->query('SELECT iduser, code, firstname, lastname, username, avatar, num_items, num_followers, num_following, validated FROM users, relations WHERE iduser=subscriber AND leader='.$D->u->iduser.' ORDER BY rltdate DESC LIMIT 0,'.$C->NUM_FOLLOWERS_PAGE);
