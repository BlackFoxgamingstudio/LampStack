...
if ($errored == 1) {
	...
	$sizeimage = $_FILES['uploaded_photo']['size'];
	$loadedtype = $_FILES['uploaded_photo']['type'];
	...
	if ($loadedtype=="image/jpeg" || $loadedtype=="image/gif" || $loadedtype=="image/png") {
		...
		if ($sizeimage<$C->SIZE_PHOTO) {
			 // move the file to the specified folder
			 if (move_uploaded_file($_FILES['uploaded_photo']['tmp_name'], $uploadfile)) {
				echo('1: '.$mfilename);
				return;
			 } else {
				echo('0: '.$this->lang('dashboard_myitems_tab2_msg7'));
				return;
			 }
		} else {
			echo('0: '.$this->lang('dashboard_myitems_tab2_msg6'));
			return;
		} 
	} else { 
		echo('0: '.$this->lang('dashboard_myitems_tab2_msg8'));
		return;
	}	
	echo("1: Ok");
	return;
